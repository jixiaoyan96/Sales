            ...
            if(!empty($service)){
                foreach ($service as $arr){
                    ...
                    if(empty($arr['cust_type_name'])){
                        $sql_calculation="select * from swoper$suffix.swo_service where company_name='".$arr['company_name']."' and cust_type='".$arr['cust_type']."' and cust_type_name=' ' and salesman='".$arr['salesman']."'  and status='N' and status_dt<'".$arr['status_dt']."'";

                    }else{
                        $sql_calculation="select * from swoper$suffix.swo_service where company_name='".$arr['company_name']."' and cust_type_name='".$arr['cust_type_name']."' and salesman='".$arr['salesman']."'  and status='N' and status_dt<'".$arr['status_dt']."'";
                    }
                    $m = Yii::app()->db->createCommand($sql_calculation)->queryAll();
                    if(!empty($m)&&count($m)==1){
                        $sum_y3[]=1;
